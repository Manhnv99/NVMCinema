...
export const ModalAddOrUpdate = ({ openModal, setOpenModal, dataDetail, render, whatAction, handleFetchPut, handleFetchPost }) => {
    ...
    const handleAddOrUpdate = () => {
        Swal.fire({
            title: whatAction === "post" ? "Bạn có chắc muốn thêm khu vực này?" : "Bạn có chắc muốn cập nhật khu vực này này?",
            icon: "question",
            ...
            if (result.isConfirmed) {
                if (whatAction === "post") {
                    ...
                    fieldsValue.areaId = dataDetail.id;
                    //fetchPut
    ...
    return (
        <>
            <Modal
                title={<Typography.Title level={3}>{whatAction === "post" ? "Thêm Khu Vực" : "Cập Nhật Khu Vực"}</Typography.Title>}
                open={openModal}
                ...
                >
                    ...
                    <div className="flex justify-end">
                        <Button type="primary" htmlType="submit">{whatAction === "post" ? "Thêm khu vực" : "Cập nhật khu vực"}</Button>
                    </div>
                </Form>
            </Modal>
        </>
